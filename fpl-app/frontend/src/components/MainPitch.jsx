...
import { getSnapshot } from "../utils/squadPrefetch"; // NEW
...
export default function MainPitch() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // ── KEY FIX: run snapshot fetch and challenge-state fetch in PARALLEL ──
        // getSnapshot(gw) resolves from the prefetch cache — usually instant.
        // api.loadChallengeState fires at the same time instead of waiting.
        const [data, serverState] = await Promise.all([
          getSnapshot(gw).then(snap => snap ?? api.optimizeSquad(100)),
          token
            ? api.loadChallengeState(token).catch(() => null)
        ...
        // Try server state first (cross-browser), then localStorage fallback
        let restored = false;
  ...
  useEffect(() => {
    ...
    const modelPlayerIds = [...squad.starters.map(p => p.player_id), ...squad.bench.map(p => p.player_id)].filter(Boolean);
    const userPlayerIds  = [...userTeam.starters.map(p => p.player_id), ...userTeam.bench.map(p => p.player_id)].filter(Boolean);
    ...
    api.getGwPoints(gw, allIds).then(pointsMap => {
      ...
      const swaps = userTeam.starters.filter(p => p._swapped).map(p => p.web_name);
