...
Fixes applied:
  1. GW cached in MongoDB (30-min TTL) — stops hitting FPL API on every request
  2. Startup background thread — pre-loads predictions + runs ILP on wake
  3. /api/warmup endpoint — point UptimeRobot HERE instead of /api/health
"""
...
# ── App + CORS ─────────────────────────────────────────────────────────────────
app = FastAPI(title="FPL AI Decision Engine", version="1.2.0")
...
# In-memory cache for ISL data
_isl_cache: dict = {}
ISL_CACHE_TTL   = 3600
ISL_LEAGUE_ID   = 323
ISL_SEASON      = 2024

# ── Startup warm-up flag ───────────────────────────────────────────────────────
_warmup_done = False


# ══════════════════════════════════════════════════════════════════════════════
# FIX 1 — GW CACHE IN MONGODB (30-min TTL)
# Stops hitting FPL API on every /api/current-gw request
# ══════════════════════════════════════════════════════════════════════════════
...
def get_current_gw_cached() -> dict:
    """
    Returns current GW data.
    Tries MongoDB cache first (30-min TTL).
    Falls back to live FPL API and updates cache.
    """
    try:
        db  = get_db()
    ...
    try:
        ...
        pass  # Non-fatal — still return fresh data

    return data


# ══════════════════════════════════════════════════════════════════════════════
# FIX 2 — STARTUP PRE-WARM (background thread on Render wake)
# Loads predictions + pre-builds ILP snapshot so first user never waits
# ══════════════════════════════════════════════════════════════════════════════

def _background_warmup():
    """
    Runs in a daemon thread immediately on startup.
    1. Pre-loads predictions into memory
    2. Fetches + caches current GW
    3. Runs ILP optimizer and saves snapshot to MongoDB
    So when the first real user arrives, everything is already hot.
    """
    global _warmup_done
    try:
        print("[WARMUP] Starting background warm-up...")

        # Step 1: Pre-load predictions
        print("[WARMUP] Loading predictions...")
        get_predictions()
        print("[WARMUP] Predictions loaded.")

        # Step 2: Cache current GW
        print("[WARMUP] Fetching current GW...")
        gw_data = get_current_gw_cached()
        gw      = gw_data["gameweek"]
        print(f"[WARMUP] Current GW: {gw}")

        # Step 3: Pre-build squad snapshot if not already cached
        try:
            db       = get_db()
            existing = db.squad_snapshots.find_one({"gw": gw})
            if existing:
                print(f"[WARMUP] Snapshot for GW{gw} already exists — skipping ILP.")
            else:
                print(f"[WARMUP] Running ILP optimizer for GW{gw}...")
                snapshot_squad(gw)
                print(f"[WARMUP] Snapshot for GW{gw} saved to MongoDB.")
        except Exception as e:
            print(f"[WARMUP] Snapshot pre-build failed (non-fatal): {e}")

        _warmup_done = True
        print("[WARMUP] Background warm-up complete. All endpoints are hot.")

    except Exception as e:
        print(f"[WARMUP] Warm-up failed (non-fatal): {e}")
        _warmup_done = True  # Mark done even on failure so status reflects reality


@app.on_event("startup")
async def startup_event():
    """Launch background warm-up thread immediately when Render starts the app."""
    t = threading.Thread(target=_background_warmup, daemon=True)
    t.start()
    print("[STARTUP] Background warm-up thread launched.")


# ══════════════════════════════════════════════════════════════════════════════
# FIX 3 — /api/warmup ENDPOINT
# Point UptimeRobot at THIS instead of /api/health
# It actively exercises the full stack: predictions + GW + snapshot
# ══════════════════════════════════════════════════════════════════════════════

@app.get("/api/warmup")
def warmup():
    """
    Point UptimeRobot at this endpoint (not /api/health).
    Actively warms: predictions cache, GW cache, and snapshot check.
    Returns status of each component so you can monitor health too.
    """
    status = {
    ...
    if status["gw"]:
        try:
            ...
            if existing:
                ...
                # Snapshot missing — build it now (UptimeRobot keeps hitting this
                # so it'll build within one ping cycle if somehow lost)
                snapshot_squad(status["gw"])
                status["snapshot_exists"] = True
...
# ── PATCHED: /api/current-gw now uses MongoDB cache ───────────────────────────
@app.get("/api/current-gw")
def current_gw():
    try:
        return get_current_gw_cached()
    except Exception as e:
...
def fetch_fpl_squad(team_id: int):
    ...
    try:
        ...
        if len(current_rows):
            current_gw_val = int(current_rows["id"].iloc[0])
        else:
...
def retrain_predictions(secret: str = ""):
    ...
    try:
        ...
        # After retrain, invalidate GW cache and rebuild snapshot for new GW
        try:
            db = get_db()
